...
- 승인 대기 목록은 요청 행·회원 정보를 일괄 읽고 30초간 서버 캐시합니다.
...
Apps Script 웹앱은 **POST → 302 → 결과 전달**의 2단계로 응답합니다. 실측하면 이렇습니다.

| 구간 | 시간 |
|---|---|
| DNS + TCP + TLS | 0.1초 |
| 1단계: 스크립트 실행 | 1.9 ~ 3.1초 |
| 2단계: 결과 전달 | 보통 0.4초, **가끔 30초 이상 멈춤** |

서버 연산 자체는 1초 미만입니다(`diagnosePendingRequestsPerformance()`로 확인 가능).
느리게 느껴지는 원인은 시트 처리가 아니라 **2단계가 간헐적으로 멈추는 것**입니다.

그래서 가벼운 조회는 대기 시간을 12초로 짧게 잡고, 넘기면 끊고 다시 보냅니다.
재시도는 보통 2~3초에 끝나므로 30초를 기다리는 것보다 훨씬 빠릅니다.
엑셀·PDF·업로드처럼 실제로 오래 걸리는 작업은 종전대로 넉넉히 기다립니다.

승인 대기 목록은 요청 건수와 payload 크기에 따라 수 초가 걸릴 수 있어 로그인에 합치지
않습니다. 대시보드를 먼저 띄우고 목록 영역에만 로딩 표시를 두므로, 목록을 기다리는 동안에도
다른 기능을 쓸 수 있습니다.

시트를 읽을 때는 **`getRange()` 호출 횟수**가 비용을 좌우합니다. 읽는 셀 수를 줄이려고
필요한 행만 골라 여러 번 읽으면 왕복이 늘어 오히려 느려지므로, 한 번에 읽는 방식을 유지합니다.
