...
function doGet(e) {
  ...
  const page = e && e.parameter ? normalize(e.parameter.page).toLowerCase() : '';
  const hasToken = !!(e && e.parameter && e.parameter.token);
  let templateName = 'index';
  let title = '회원관리 시스템';
  if (page === 'photo') {
    templateName = 'photo';
    title = '회원 사진 업로드';
  } else if (page === 'membercard') {
    templateName = 'membercard';
    title = '회원 카드';
  } else if (page === 'reconsent' || hasToken) {
    templateName = 'reconsent';
  }
...
function getPendingRequestRows_(reqSheet, numCols) {
  ...
  const statusRange = reqSheet.getRange(2, 3, lastRow - 1, 1);
  let rowNumbers = [];
  try {
    const found = statusRange.createTextFinder('REQUESTED').matchEntireCell(true).findAll();
    rowNumbers = found.map(r => r.getRow());
  } catch (e) {}
  ...
  rowNumbers.sort((a, b) => b - a).forEach(rowNum => {
    ...
    out.push({
      row_idx: rowNum,
      row: reqSheet.getRange(rowNum, 1, 1, cols).getValues()[0]
    });
  });
  return out;
}
...
function submitRequest(form) {
  ...
  reqSheet.appendRow([requestId, form.type, 'REQUESTED', safeId, safeName, form.current_dept_id || '', form.target_dept_id || '', form.reason || '', JSON.stringify(form), photoFileId, form.requester_email, form.requester_name, nowStr, '', '', formFileId]);
  return "신청 완료";
}
...
function getPendingRequests() {
  ...
  const reqSheet = ss.getSheetByName(SHEETS.REQUESTS);
  if (!reqSheet) return [];
  if (reqSheet.getLastRow() < 2) return [];
  ...
  if (needMemberInfo) {
    const memSheet = ss.getSheetByName(SHEETS.MEMBERS);
    if (memSheet) {
      memberMap = getColumnMap(memSheet);
      if (memberMap[COLS.ID] !== undefined) {
        const targetIds = Object.keys(pendingMemberIds);
        if (targetIds.length <= 30) {
          const lastCol = memSheet.getLastColumn();
          for (let i = 0; i < targetIds.length; i++) {
            const memberId = targetIds[i];
            const rowIdx = findMemberRowById_(memSheet, memberMap, memberId);
            if (rowIdx < 2) continue;
            const r = memSheet.getRange(rowIdx, 1, 1, lastCol).getValues()[0];
            const dId = memberMap[COLS.DEPT_ID] !== undefined ? String(r[memberMap[COLS.DEPT_ID]]).trim() : '';
            memInfo[memberId] = {
              age: r[memberMap[COLS.AGE]] || calculateAge(r[memberMap[COLS.BIRTH]]),
              rank: r[memberMap[COLS.RANK]],
              dept_id: dId,
              row: r
            };
          }
